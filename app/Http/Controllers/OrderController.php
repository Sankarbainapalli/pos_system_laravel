       ...
       $order->save();
        ...
        if(count($request->item_name)>0)
        {
            ...
            if(Auth::user()->role_id=="SUPERADMIN"){
              ...
              $total_stock=Stock::where('product_id',$request->item_name[$item])->sum('qty');
          
             $update_stock=$total_stock-($request->qty[$item]/1000);

           Stock::where('product_id', $request->item_name[$item])->update(['qty' => ($update_stock)]);
       ...
       $order->save();
        ...
        if(count($request->item_name)>0)
        {
            ...
            if(Auth::user()->role_id=="SUPERADMIN"){
             ...
             $update_stock=$total_stock-($request->qty[$item]);
...
curl_close($ch);
    ...
    return redirect()->route('pos/regular_invoice_print', ['id' => $order->id]);
